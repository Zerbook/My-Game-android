...
public class GameScreen extends Screen {
    ...
    private void updateReady(List<TouchEvent> touchEvents) {
        Log.d("mrnom", "GameScreen_updateReady");
        if(touchEvents.size() > 0)
            state = GameState.Running;
    }
    private void updateRunning(List<TouchEvent> touchEvents, float deltaTime) {
        Log.d("mrnom", "GameScreen_updateRunning");
    ...
    private void updatePaused(List<TouchEvent> touchEvents) {
        Log.d("mrnom", "GameScreen_updatePaused");
        int len = touchEvents.size();
        for(int i = 0; i < len; i++) {
            TouchEvent event = touchEvents.get(i);
            if(event.type == TouchEvent.TOUCH_UP) {
    ...
    private void updateGameOver(List<TouchEvent> touchEvents) {
        Log.d("mrnom", "GameScreen_updateGameOver");
        int len = touchEvents.size();
        for(int i = 0; i < len; i++) {
            TouchEvent event = touchEvents.get(i);
            if(event.type == TouchEvent.TOUCH_UP) {
    ...
    @Override    public void present(float deltaTime) {
        Log.d("mrnom", "GameScreen_present");
    ...
    private void drawWorld(World world) {
        Log.d("mrnom", "GameScreen_drawWorld");
        Graphics g = game.getGraphics();
    ...
    private void drawReadyUI() {
        Log.d("mrnom", "GameScreen_drawReadyUI");
        Graphics g = game.getGraphics();
        g.drawPixmap(Assets.ready, 47, 100);
        g.drawLine(0, 416, 480, 416, Color.BLACK);
    }
    private void drawRunningUI() {
        Log.d("mrnom", "GameScreen_drawRunningUI");
        Graphics g = game.getGraphics();
    ...
    private void drawPausedUI() {
        Log.d("mrnom", "GameScreen_drawPausedUI");
        Graphics g = game.getGraphics();
        g.drawPixmap(Assets.pause, 80, 100);
        g.drawLine(0, 416, 480, 416, Color.BLACK);
    }
    private void drawGameOverUI() {
        Log.d("mrnom", "GameScreen_drawGameOverUI");
        Graphics g = game.getGraphics();
    ...
    public void drawText(Graphics g, String line, int x, int y) {
        Log.d("mrnom", "GameScreen_drawText");
        int len = line.length();
        for (int i = 0; i < len; i++) {
